...
## 2025-02-14 - Information Leakage through Generic Exception Text
**Vulnerability:** In `src/#ctdi#print_repair.prog.abap`, the root exception `cx_root` text (`lx_root->get_text( )`) was caught and directly exposed to the UI variable `lv_emsg` inside an unhandled error catch block.
**Learning:** Returning unhandled exception strings directly to the frontend allows internal execution flow, ABAP technical error details, or database specifics to leak to users.
**Prevention:** Unhandled technical exceptions should only have their details logged internally (e.g., via `log_exception( lx_root )`), while generic strings (e.g., 'An unexpected system error occurred') should be presented to the user.
