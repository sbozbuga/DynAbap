...
## 2026-09-06 - [Information Exposure through Custom Exceptions in Catch Blocks]
**Vulnerability:** Raw exception text from custom exceptions like `/ctdi/cx_no_config_found`, `/ctdi/cx_print_driver_error` and generic `cx_root` were directly assigned to UI variables and exposed to the end user.
**Learning:** Returning `get_text()` output on exceptions—even custom business exceptions—to the front end can expose technical specifics about system configuration, class names, or table paths, aiding attackers.
**Prevention:** Instead of directly binding exception texts to UI elements, catch the exceptions, log them securely on the backend (e.g. `log_exception( lx )`), and replace the UI string with a sanitized, hardcoded standard message.
