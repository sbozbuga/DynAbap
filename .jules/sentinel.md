...
## 2025-02-14 - Information Leakage through Exception Attribute
**Vulnerability:** In `src/#ctdi#print_repair.prog.abap`, custom exception attributes and `cx_root` exception texts were being directly assigned to `lv_emsg` and outputted via `MESSAGE`, potentially leaking technical backend details to the UI.
**Learning:** Even custom exceptions (like `/ctdi/cx_print_driver_error` or `/ctdi/cx_no_config_found`) may contain technical error messages intended for backend tracing rather than UI exposure. Passing exception texts directly into ABAP UI messages compromises security.
**Prevention:** Always log exceptions securely on the backend (using `/ctdi/cl_print_driver_log=>log_exception`) and assign a sanitized, generic error message string to the UI variable `lv_emsg`.
