...
## 2025-02-14 - Information Leakage through Raw Exception Text to UI
**Vulnerability:** Raw exception text from custom exceptions (`/ctdi/cx_no_config_found`, `/ctdi/cx_print_driver_error`) and technical exceptions (`cx_root`) was directly assigned to a message variable and displayed to the UI in `src/#ctdi#print_repair.prog.abap`.
**Learning:** Exposing `get_text()` output from exceptions directly to end users can leak internal system architecture, missing configuration details, or database state.
**Prevention:** Always log exceptions securely on the backend (e.g., via `/ctdi/cl_print_driver_log=>log_exception()`) and present a generic, sanitized error message string to the user.
