...
**Vulnerability:** Raw exception text (`lx_...->get_text( )`) was exposed to users in UI error messages.
**Learning:** Exposing internal exception text can leak system details and architectural structure.
**Prevention:** Always log the exception object securely via `/ctdi/cl_print_driver_log=>log_exception( )` and return a sanitized, generic error message (e.g., 'An unexpected system error occurred') to the user.
