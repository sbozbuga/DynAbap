...
## 2025-02-14 - Information Exposure through Direct Exception Message Output
**Vulnerability:** In `src/#ctdi#print_repair.prog.abap`, the unhandled exceptions (`/ctdi/cx_no_config_found`, `/ctdi/cx_print_driver_error`, and `cx_root`) were outputting their raw exception texts directly to the user interface via `lv_emsg = lx...->get_text()`.
**Learning:** Exception text objects can contain technical error messages (e.g., "Cannot instantiate class X") that expose internal structure and class names meant for backend tracing. Passing exception texts directly into UI messages compromises security and can leak table, configuration, or structural names.
**Prevention:** Sanitize UI error messages in ABAP dialogs. Instead of embedding custom exception attributes, log technical details securely using a standardized logging layer (e.g., `/ctdi/cl_print_driver_log=>log_exception`), and issue a generic, safe string like `TEXT-007` to the frontend user.
