...
## 2025-02-14 - Information Exposure through Program Exceptions
**Vulnerability:** Raw exception text from `cx_root` and custom exceptions (like `/ctdi/cx_no_config_found`) was being outputted directly to the UI ALV grid (via `msg` fields) or standard `MESSAGE` statements in reporting programs.
**Learning:** Passing detailed exception text to the frontend in report ALVs or UI messages leaks technical implementation details (table names, variable states, ads traces, class structures).
**Prevention:** Catch blocks in report programs should log the full exception to the backend using a centralized logging class (e.g. `/ctdi/cl_print_driver_log=>log_exception()`) and populate the UI error fields with a generic sanitized literal such as `'An unexpected system error occurred'`.
