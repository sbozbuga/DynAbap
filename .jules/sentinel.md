...
## 2024-07-18 - Information Leakage via Error Messages
**Vulnerability:** Direct exposure of `cx_root` exception texts to the UI using `lx_root->get_text()`.
**Learning:** This existed because catch-all exception blocks often naively display the system error text to inform the user, but this can inadvertently leak system internals, database schema details, or code paths to end users.
**Prevention:** Never embed generic exception texts (like `cx_root`) directly into string templates or messages that will be displayed in the UI. Always log the exception securely using a backend logger (e.g., `/ctdi/cl_print_driver_log=>log_exception( lx_root )`) and provide a sanitized, generic error message to the user.
