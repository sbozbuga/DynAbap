...
## 2025-02-14 - Selective Exception Text Masking
**Vulnerability:** Masking all custom business exception messages with generic errors in an attempt to prevent information leakage degrades UI feedback.
**Learning:** In internal SAP/ABAP applications, custom exceptions (`cx_cust_error`, `cx_print_driver_error`) are designed to return safe, actionable feedback to users. Only unhandled, system-level technical exceptions (`cx_root`) pose a risk of leaking internal details (like stack traces or SQL errors) and need to be sanitized for the UI.
**Prevention:** Differentiate between business exceptions and system exceptions in catch blocks. Pass custom exception texts to the UI as intended, but sanitize and log the generic `cx_root` exception text.
