...
## 2025-02-14 - Prevent UI Exposure of `cx_root` Exception Text
...
**Learning:** Exposing detailed technical errors directly to users allows for potential information leakage regarding internal system or database states. Using hardcoded safe strings is a viable solution if text symbols are missing.
**Prevention:** Always log exceptions securely on the backend (e.g., using `/ctdi/cl_print_driver_log=>log_exception( lx_err )`) and display a sanitized, generic error message string to the user.
