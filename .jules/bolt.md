...
## 2026-06-21 - Prevent O(N*M) nested loop lookups with BINARY SEARCH
...
## 2026-06-26 - Eliminate Internal Table Allocation Overhead
...
## 2026-07-07 - Critical Issue with Deduplication before Binary Search
**Learning:** A previous optimization added `BINARY SEARCH` to a `READ TABLE` statement inside a loop. However, the data setup logic prior to the loop used `DELETE ADJACENT DUPLICATES` without first sorting the internal table by the comparison fields. `DELETE ADJACENT DUPLICATES` requires the table to be sorted by the compared fields; otherwise, it only removes identical rows that happen to be next to each other, failing to fully deduplicate the table. Furthermore, `BINARY SEARCH` *strictly* requires the table to be sorted. Therefore, failing to `SORT` before `DELETE ADJACENT DUPLICATES` not only results in an incorrectly sized, duplicate-filled table in memory, but also totally breaks the binary search functionality, causing erratic and missing lookup results.
**Action:** Always ensure that before using `DELETE ADJACENT DUPLICATES`, the internal table is explicitly `SORT`ed by the exact fields being compared. This is critical both for ensuring accurate O(N log N) deduplication and for preparing the table for safe, performant O(log N) `BINARY SEARCH` operations later.
