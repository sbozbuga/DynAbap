...
## 2025-02-12 - Consolidate equi and mara sequential DB lookups
**Learning:** Sequential DB checks where a missing part number lookup (`equi`) triggers a secondary material number lookup (`mara`) cause unnecessary latency.
**Action:** Use a single `LEFT OUTER JOIN` between `equi` and `mara` to fetch both values simultaneously, reducing DB hits significantly during fallback evaluations.
