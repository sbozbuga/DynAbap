...
**Learning:** Sequential `SELECT SINGLE` queries on different tables (like `equi` and `mara`) that share a common key (like `matnr`) can be combined to reduce N+1-style DB roundtrips.
**Action:** Use `LEFT OUTER JOIN`s to fetch all dependent properties simultaneously in a single query by linking the tables on the common key, reducing overhead significantly. Keep the original fallback logic checking for intermediate existing values using outer IF/ELSE wrappers.
