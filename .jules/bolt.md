...
## 2026-07-06 - Sort before DELETE ADJACENT DUPLICATES
**Learning:** In ABAP, the `DELETE ADJACENT DUPLICATES` statement requires the internal table to be sorted by the fields being compared beforehand. If it is not sorted, the deduplication will be incomplete, leading to larger memory consumption, performance issues, and potential logical bugs in downstream logic (e.g. failing BINARY SEARCH).
**Action:** Always verify that an internal table is `SORT`ed by the correct fields before applying `DELETE ADJACENT DUPLICATES`.
