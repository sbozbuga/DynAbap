...
**Learning:** Sequential `SELECT SINGLE` queries on different tables (like `equi` and `mara`) that share a relationship (e.g., `equi-matnr` to `mara-matnr`) can be combined to reduce N+1-style DB roundtrips in fallback routines.
**Action:** Use `LEFT OUTER JOIN`s to fetch all dependent properties simultaneously in a single query, preserving conditional checks on intermediate variables if necessary.
