...
## 2026-08-01 - Add text pool entries for NO-DISPLAY background parameters
**Learning:** Background selection screen parameters defined with `NO-DISPLAY` (e.g., `P_SF`) still require text descriptions in the text pool. Without them, background job scheduling screens, parameter variants, and dynamic UIs lack proper labeling, reducing accessibility for administrators.
**Action:** Always add text descriptions for `NO-DISPLAY` parameters in the program's XML text pool (e.g., `<TPOOL>` and `<I18N_TPOOL>`) to ensure proper labeling in administrative UIs and improve accessibility.
