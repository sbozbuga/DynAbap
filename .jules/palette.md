...
## 2026-08-01 - Add missing selection screen text pool entries for NO-DISPLAY parameters
**Learning:** Background selection screen parameters defined with `NO-DISPLAY` (e.g., `P_SF`) are not visible on the main UI but are still visible in dynamic UIs, variant selection, and background job scheduling screens. If their descriptions are missing from the program's text pool (`<TPOOL>` and `<I18N_TPOOL>`), users see confusing technical parameter names instead of human-readable labels.
**Action:** Always verify that even `NO-DISPLAY` parameters have corresponding text entries defined in the `.prog.xml` file. If they are missing, add them (using confirmed fallback text from inline comments if a translation is missing) to improve UX for administrators and resolve `selection_screen_texts_missing` abaplint warnings.
