...
CLASS /CTDI/CL_PRINT_DATA_LEGACY IMPLEMENTATION.
  ...
  METHOD determine_change_doc_data.
    " ⚡ Bolt Optimization: Consolidate sequential equi and equz lookups to reduce DB round-trips
    " Pre-fetch current values as defaults
    SELECT SINGLE e~serge, e~matnr, z~mapar
      FROM equi AS e
             LEFT OUTER JOIN
               equz AS z ON z~equnr = e~equnr
