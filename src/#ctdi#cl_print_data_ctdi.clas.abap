...
CLASS /CTDI/CL_PRINT_DATA_CTDI IMPLEMENTATION.
  ...
  METHOD get_repair_result.
    ...
    IF mv_kdauf IS NOT INITIAL.
      " ⚡ Bolt Optimization: Consolidate sequential document flow lookups into a single DB hit
      SELECT SINGLE a~vgbel, b~vbtyp
        FROM vbak AS a
        LEFT OUTER JOIN vbak AS b ON b~vbeln = a~vgbel
        WHERE a~vbeln = @mv_kdauf
        INTO @DATA(ls_vbak).

      lv_contract = ls_vbak-vgbel.
      IF lv_contract IS NOT INITIAL AND ls_vbak-vbtyp <> 'G'.
        CLEAR lv_contract.
      ENDIF.
    ENDIF.
