"! CTDI Print Data Provider Extension.
"! Extends the base Alcatel class to read repair results from the new
"! /CTDI/REP_RESULT table using an 11-step access sequence based on Contract, SKZ, AKZ, and Swap Flag.
CLASS /ctdi/cl_print_data_ctdi DEFINITION
  ...
  PROTECTED SECTION.
    " Redefined repair result retrieval:
    " First fetches the contract (vgbel) from VBAK. Then reads operation 9010
    " from AFRU to get SKZ. Evaluates an access sequence against /CTDI/REP_RESULT.
...
CLASS /CTDI/CL_PRINT_DATA_CTDI IMPLEMENTATION.
  ...
  METHOD get_repair_result.
    ...
    IF mv_kdauf IS NOT INITIAL.
      " ⚡ Bolt Optimization: Consolidate sequential vbak lookups into a single DB hit
      SELECT SINGLE v1~vgbel, v2~vbtyp
        FROM vbak AS v1
               LEFT OUTER JOIN
                 vbak AS v2 ON v2~vbeln = v1~vgbel
        WHERE v1~vbeln = @mv_kdauf
        INTO @DATA(ls_vbak_join).

      IF sy-subrc = 0 AND ls_vbak_join-vgbel IS NOT INITIAL.
        lv_contract = ls_vbak_join-vgbel.
        " Verify the linked document is actually a contract (vbtyp = 'G')
        IF ls_vbak_join-vbtyp <> 'G'.
          CLEAR lv_contract.
        ENDIF.
      ENDIF.
    ENDIF.
    ...
    IF lv_contract IS NOT INITIAL.
      ...
      APPEND VALUE #( vbeln      = lv_contract
                      bemot      = lv_bemot
                      akz        = mv_qmcod
                      tauschfall = mv_swap_flag ) TO lt_steps.
      " 2. Kontrakt + SKZ + AKZ
      APPEND VALUE #( vbeln      = lv_contract
                      bemot      = lv_bemot
                      akz        = mv_qmcod
